...
if [[ -e $indir/${preprocfeat} ]]; then
  if [[ $overwriteFlag == 1 ]]; then
    ...
    #t1Skull=`cat $indir/rsParams | grep "t1Skull=" | tail -1 | awk -F"=" '{print $2}'`
    #fslmaths $t1Skull $regDir/highres_head.nii.gz
    #t1WM=`cat $indir/rsParams | grep "t1WMedge=" | tail -1 | awk -F"=" '{print $2}'`
    #fslmaths $t1WM $regDir/example_func2highres_fast_wmedge.nii.gz
    ...
    #fslmaths $FSLDIR/data/standard/MNI152_T1_2mm.nii.gz $regDir/standard_head.nii.gz
    #fslmaths $FSLDIR/data/standard/MNI152_T1_2mm_brain_mask_dil.nii.gz $regDir/standard_mask.nii.gz
