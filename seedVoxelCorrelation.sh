...
function extractROIts()
{
  ...
  clobber "${roiOutDir}/${roiName}_standard.nii.gz" &&\
	...
	if [[ $seedVol == 0.000000 ]]; then
		...
		if [[ $motionscrubFlag == 0 ]]; then
			awk '{print $1}' "${roiOutDir}/${roiName}"_residvol_ts.txt > "${roiOutDir}"/temp_"${roiName}"_residvol_ts.txt
			mv "${roiOutDir}"/temp_"${roiName}"_residvol_ts.txt "${roiOutDir}/${roiName}"_residvol_ts.txt
		elif [[ $motionscrubFlag == 1 ]]; then
			awk '{print $1}' "${roiOutDir}/${roiName}"_residvol_ts.txt > "${roiOutDir}"/temp_"${roiName}"_residvol_ts.txt
			awk '{print $1}' "${roiOutDir}/${roiName}"_residvol_ms_ts.txt > "${roiOutDir}"/temp_"${roiName}"_residvol_ms_ts.txt
			mv "${roiOutDir}"/temp_"${roiName}"_residvol_ts.txt "${roiOutDir}/${roiName}"_residvol_ts.txt
 ...
 function corrmap2mni()
  {
    ...
    if [[ $motionscrubFlag == 0 ]]; then
      ...
      yMax=$(sort -r "${roiOutDir}"/"${roi}"_residvol_ts.txt | tail -1 | awk '{print ($1+($1*0.0022))}')
      yMin=$(tail -1 "${roiOutDir}"/"${roi}"_residvol_ts.txt | awk '{print ($1-($1*0.0022))}')
      ...
      scrubDataCheck=$(head -1 "${rawEpiDir}"/motionScrub/deleted_vols.txt)
      ...
      if [[ $scrubDataCheck != "" ]]; then
        ...
        # Creating new plots with fsl_tsplot
        # ~2.2% plotting difference between actual Ymin and Ymax values (higher and lower), with fsl_tsplot
        yMax=$(sort -r "${roiOutDir}"/"${roi}"_residvol_ms_ts.txt | tail -1 | awk '{print ($1+($1*0.0022))}')
        yMin=$(tail -1 "${roiOutDir}"/"${roi}"_residvol_ms_ts.txt | awk '{print ($1-($1*0.0022))}')
...
seedTestBase=$(grep "seeds=" "$rawEpiDir"/rsParams | awk -F"=" '{print $2}' | awk -F"-r " '{for (i=2; i<=NF; i++) print $i}')
